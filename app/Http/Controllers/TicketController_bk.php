...
class TicketController extends Controller
{
    ...
    public function create()
    {
        if( (Auth::user()->role == 'ticket_admin') || (Auth::user()->role == 'agent') ) {
            ...
            $addOnList = AddOn::where('status', 'Active')->pluck('name', 'id');
            ...
            return view('ticket.create', compact('ticketTypeList', 'userList', 'subjectList', 'paymentMethodList', 'productGroupList', 'sweetnessList', 'iceList', 'productList', 'addedList', 'categoryList', 'addOnList'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if (count($cart)) {
            ...
            foreach ($cart as $item) {
                ...
                array_push($productArray, [
                    ...
                    'Sweetness' => $item->options->level_of_sweetness,
                    'Ice' => $item->options->level_of_ice,
                    'AddOnName' => $item->options->add_on_name,
                    'AddOnSubtotal' => $item->options->add_on_subtotal,
                    'ExAdd' => $item->options->ex_add,
                    'ExPrice' => $item->options->ex_price,
                ]);

                $productPrice = $productPrice + $item->subtotal;
                $addOnPrice = $addOnPrice + $item->options->add_on_subtotal;
                $exPrice = $exPrice + $item->options->ex_price;
            }

            $totalPrice = $productPrice + $addOnPrice + $exPrice;
        ...
        if ($request->discount_per == '') {
            $discountPer = 0;
        } else {
            $discountPer = $request->discount_per;
        }

        $totalDiscount = (($totalPrice * $discountPer) / 100);
        ...
        $ticket->product_price = $productPrice;
        $ticket->add_on_price = $addOnPrice;
        $ticket->ex_price = $exPrice;
        $ticket->total_price = $totalPrice;
        $ticket->discount_per = $request->discount_per;
        $ticket->total_discount = $totalDiscount;
        $ticket->discount_price = $totalPrice - $totalDiscount;
        ...
        if (count($cart)) {
            ...
            foreach ($cart as $item) {
                ...
                $orderDetail->product_group = $item->options->group;
                $orderDetail->product_quantity = $item->qty;
                $orderDetail->product_price = $item->price;
                $orderDetail->product_subtotal = $item->subtotal;
                $orderDetail->level_of_sweetness = $item->options->level_of_sweetness;
                $orderDetail->level_of_ice = $item->options->level_of_ice;
                $orderDetail->add_on_name = $item->options->add_on_name;
                $orderDetail->add_on_subtotal = $item->options->add_on_subtotal;
                $orderDetail->ex_add = $item->options->ex_add;
                $orderDetail->ex_price = $item->options->ex_price;
    ...
    {   
        ...
        if ($ticketType == 1) {
            $ccUserList = User::whereIn('id',[3])->pluck('name', 'id');
    ...
    public function addToCart(Request $request)
    {
        ...
        if (isset($addOn)) {
            ...
            $addOnSubtotal = $addOn->price * $request->quantity;
            $rand = rand();
        } else {
            $addOnName = '';
            $addOnSubtotal = 0;
        ...
        $group = $request->group;
        $levelOfSweetness = $request->level_of_sweetness;
        $levelOfIce = $request->level_of_ice;
        $exAdd = $request->ex_add;
        if ($request->ex_price == '') {
            $exPrice = 0;
        } else {
        ...
        Cart::instance($this->productCart)->add([
                      ...
                      'options' => ['group' => $group, 'level_of_sweetness' => $levelOfSweetness, 'level_of_ice' => $levelOfIce, 'add_on_name' => $addOnName, 'add_on_subtotal' => $addOnSubtotal, 'rand' => $rand, 'ex_add' => $exAdd, 'ex_price' => $exPrice]
                  ]);
